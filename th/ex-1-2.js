...
function sumOddNumber(numbers) {
    let filteredArray = numbers.filter((number) => number%2 !== 0 && (number%3 === 0 || number%5 === 0) && number%11 !== 0);
    let sumNumbers = filteredArray.reduce((acc, curr) => acc + curr, 0);
    return `${sumNumbers} จาก [${filteredArray}]`;
}
